...
type ProgInfo = ((Time,Int),		-- Sampling info
		 PlotSpec,	        -- Plotting info
		 [(Int,Proc)])		-- Initial processes

-- BCP: Changed so that it doesn't bother returning the next unique name,
-- since these are now being allocated differently.
compileProgram :: [Decl] 
               -> Int               -- "Base" to use for unique names
...
compileProgram ds b
  ...
  where
    ...
    gid_env = Map.fromList $
	      [ (s, Gbl (mkGblId u s))
	      | (u,s) <- (uniqueFrom (firstUnique b)) `zip` binders]

    gbl_env  = initCM init_cxt (compileDecls ds)
    init_cxt = initCxt gid_env gbl_env b
  ...
  where
	...
	where
	   cxt' = cxt { cm_idenv = foldl add (cm_idenv cxt) lcls, 
		  	cm_uniq = nextUnique uniq (length ss) }
	   uniq = cm_uniq cxt
	   lcls = [LclId u s | (u,s) <- (uniqueFrom uniq) `zip` ss]
