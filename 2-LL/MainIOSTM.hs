...
createThread numOps tList maxNumber mvar = forkIO $ do
	callNTimes numOps $ do
		...
		atomically $ LL.deleteListNode tList rnd1
...
main1 numops listLength numThreads = do
	ourList <- atomically (LL.createSampleList (reverse [x | x <- [1 .. listLength], ((mod x 2) == 0)]))
	-- ourListAsString <- atomically (LL.toString ourList)
	timeStart <- getClockTime
	ourTList <- newTVarIO ourList
	-- listAsString <- atomically (LL.toString ourList)
	...
	-- listAsString <- atomically (LL.toString ourList2)
