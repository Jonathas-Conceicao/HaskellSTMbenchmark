...
--this benchmark will attempt to compare a two-threaded list with conflicts with one without conflicts.
 ...
 createThread numOps tList maxNumber mvar =
 	 forkIO ( do
			{ callNTimes numOps 
				...
				; case rnd1 of
				; 1 -> do {atomically (deleteListNode tList rnd2)
									; return ()}
				; 2 -> do {atomically (insertListNode tList rnd2)
									; return ()}
				; otherwise -> do {atomically (lookupListNode tList rnd2)
									; return ()}
				}) 
				; putMVar mvar 1
			})
 ...
 main1 numops listLength numThreads = do
	...
	; ourTList <- atomically (newTVar ourList)
