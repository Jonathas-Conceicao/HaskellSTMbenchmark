...
-- import Text.Printf
 ...
 lookupListNode :: TVar ListNode -> Int -> STM ()
 lookupListNode curTNode numberToLookup =
	do
	{ curNode <- readTVar curTNode
	; let nextNTNode = nodenextN curNode
	; nextNNode <- readTVar nextNTNode
  ; case nextNNode of
		  ...
		  Node {val = valnextNNode, nextN = nextNnextNTNode} ->
		   do
		    { if (valnextNNode == numberToLookup)
		     	then
			   return ()--(True)
			else if (valnextNNode > numberToLookup)
			       then return ()--(False) --doLookup nextNNode
		               else do
					{ --unreadTVar curTNode
					; lookupListNode nextNTNode numberToLookup
					}
		    }
	}
