...
data ListNode = Node { val :: Int, nextN :: TVar ListNode} | Start {nextN :: TVar ListNode} | Nil
	deriving (Eq)


nodeVal :: ListNode -> Int
nodeVal Node {val = curVal} = curVal
...
insertListNode curTNode numberToInsert = do
	curNode <- readTVar curTNode
	let nextNTNode = nodenextN curNode
	let doInsertion nextNNode =	do
	...
	case nextNNode of
		...
		Node {val = valnextNNode, nextN = nextNnextNTNode} -> do
...
deleteListNode curTNode numberToDelete = do
	curNode <- readTVar curTNode
	let nextNTNode = nodenextN curNode
	nextNNode <- readTVar nextNTNode
	case nextNNode of
		Nil -> return ()
		Node {} -> do
			let nextNVal = nodeVal nextNNode
...
findListCountSteps Start {nextN = tnextN} element steps	= do
	nextNNode <- readTVar tnextN
	findListCountSteps nextNNode element steps
findListCountSteps Node {val = curElem, nextN = tnextN} element steps = do
	if (curElem == element)
		then
			return (True, steps)
		else
...
toString Start {nextN = tnextN}	= do
	nextNNode <- readTVar tnextN
	rest <- toString nextNNode
	return ("START " ++ rest)
toString (Node{val = value, nextN = tnextN}) = do
	nextNNode <- readTVar tnextN
	rest <- toString nextNNode
