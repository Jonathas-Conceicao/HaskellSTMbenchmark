...
--this benchmark will attempt to compare a two-threaded Tree with conflicts with one without conflicts.
 ...
 createThread numOps tTree maxNumber mvar =
 	 forkIO ( do
			{ callNTimes numOps 
				...
				; case rnd1 of
				; 1 -> do {atomicallyProfiled (deleteTree tTree rnd2) 1#
									; return ()}
				; 2 -> do {atomicallyProfiled (insertTree tTree rnd2) 2#
									; return ()}
				; otherwise -> do {atomicallyProfiled (lookupTree tTree rnd2) 3#
									; return ()}
				}) 
				; putMVar mvar 1
			})
 ...
 main1 numops treeSize numThreads = do
	{ ourTTree <- atomicallyProfiled(newTVar Nil) 4# 
	; atomicallyProfiled (createSampleTree [x | x <- [1 .. treeSize], ((x `mod` 2) == 0)] ourTTree) 5#
	; ourTree <- atomicallyProfiled (readTVar ourTTree) 6#
	; --ourTreeAsString <- atomically (toString ourTree)

	; timeStart <- getClockTime
	; ourTTree <- atomicallyProfiled(newTVar ourTree) 7#
	...
	; ourTree2 <- atomicallyProfiled (readTVar ourTTree) 8#
	; treeAsString <- atomicallyProfiled (toString ourTree2) 9#
