...
data Tree = Node { val :: Int, right :: TVar Tree, left :: TVar Tree } | Nil
	deriving (Eq)

newTreeNode :: Int -> Tree -> Tree -> STM (Tree)
newTreeNode number left right = do
...
createSampleTree theList tTree = do
...
findRightOfGreatestNode curTNode = do
	curNode <- readTVar curTNode
	if (curNode == Nil)
...
combine rightTNode leftTNode = do
...
deleteTree curTNode numberToDelete = do
	curNode <- readTVar curTNode
	case curNode of
		Nil -> return ()
		Node {val = value, right = rightNode, left = leftNode} -> do
...
lookupTree curTNode numberToLookup = do
	curNode <- readTVar curTNode
	case curNode of
		Nil -> return ()
		Node {val = value, right = rightNode, left = leftNode} -> do
...
toString Nil = return ""
toString Node {val = value, right = rightNode, left = leftNode}	= do
