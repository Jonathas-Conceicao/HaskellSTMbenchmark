...
createThread numOps tTree maxNumber mvar = forkIO $ do
	callNTimes numOps $ do
		rnd1 <- randomRIO (1::Int, 10)
		rnd2 <- randomRIO (1::Int, maxNumber)
		case rnd1 of
			1 -> do
				atomically (deleteTree tTree rnd2)
				return ()
			2 -> do 
				atomically (insertTree tTree rnd2)
				return ()
			otherwise -> do
				atomically (lookupTree tTree rnd2)
				return ()
	putMVar mvar 1


createThreads :: Int -> Int -> TVar Tree -> Int -> [MVar Int] -> IO()
createThreads n numOps tTree maxNumber mvars = mapM_ (createThread numOps tTree maxNumber) mvars

callNTimesDeterministic :: Int -> Int -> TVar Tree -> Int -> IO ()
callNTimesDeterministic 0 _ _ _ = return()
callNTimesDeterministic numOps maxNumber tTree discriminant = do
...
deterministicThread numOps tTree maxNumber mvar discriminant = forkIO $ do
 	callNTimesDeterministic numOps maxNumber tTree discriminant
	putMVar mvar 1


deterministicThreads :: Int -> Int -> TVar Tree -> Int -> [MVar Int] -> IO()
deterministicThreads n numOps tTree maxNumber mvars = do
